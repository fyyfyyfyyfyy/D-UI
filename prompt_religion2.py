...
def ReligionPipeline(user, question, prompt) -> Religion:
    ...
    chat_history = [{"role": "system", "content": condition1},
    ...
    prompt2 = """确定了初级欲望后,现在有了一个更加细粒度的欲望列表,你必须根据最近一次对话的事件从该列表中选一个最合适的,\
        输出格式必须为字典格式,key分别为"desire","id","value"(desire表示欲望名,id取自desire对应的标识,value表示正向或负向)，现在的欲望列表如下:"""
    ...
    # ans2_id = ans2['id']
    ans2_value = ans2["value"]
    print(f"本次推理从{low_desire}中选取了{high_desire}作为欲望")
    res_religion = Religion(desire_name=high_desire, valence=ans2_value)
            ...
            因此你的任务是根据我给的一句话,必须从下面所给的29个初级欲望中选择最符合的一个,并输出\
                该语句对应的欲望是正向还是负向(正向表示为true,负向表示为false,\
                    "我想吃"对于欲望"吃"就是正向的),你的输出形式为字典形式,key分别为\
                        "desire","value"(desire表示欲望名,value表示正向或负向)
                    如{"desire": "吃", "value": true},注意,\
                        你的输出仅包含最终的字典。29个初级欲望列表如下:"""

        start = time.time()
        answer = ReligionPipeline(user='user', question=question,
                                  prompt=prompt)
        end = time.time()
        print(answer)
